   ...
   */
	can(rule) {
		...
		if (/[A-Z0-9]:[A-Z0-9]/i.test(rule)) {
			...
			throw new Error(
				'Please provide the rule in the correct format. Rule should be in the format: `type:action` Ex: `Blog:delete` '
			);
		}
	}
...
function generateToken($data) {
	if (!$data || typeof $data !== 'object') {
		throw new Error('Please provide the data for encoding');
	}
	return jwt.sign({ data: $data }, 'xperience', {
		expiresIn: '30m',
